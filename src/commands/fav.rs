...
command!(tags(ctx, msg, _args) {
    ...
    let user_favs = favs.filter(user_id.eq(*msg.author.id.as_u64() as i64)).load::<Fav>(&conn).expect("could not retrieve favs");
    let mut fav_tags = Tag::belonging_to(&user_favs).load::<Tag>(&conn).expect("could not retrieve tags");
