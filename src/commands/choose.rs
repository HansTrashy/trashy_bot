...
pub fn choose(ctx: &mut Context, msg: &Message, args: Args) -> CommandResult {
    ...
    if args.len() < 2 {
        return match msg.channel_id.say(&ctx.http, "You have to give at least 2 options") {
            Ok(_msg) => Ok(()),
            Err(e) => {
    ...
    match msg.channel_id.say(&ctx.http, content_safe(&ctx.cache, &sanitize_for_other_bot_commands(&chosen), &settings)) {
        Ok(_msg) => Ok(()),
        Err(e) => {
            error!("Failure sending message: {:?}", e);
            Err(e.into())
